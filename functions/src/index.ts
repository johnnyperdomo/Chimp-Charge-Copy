...
//FIX: make sure that all server functions that write to firestore are idempotent, and really fail-safe type code, to prevent duplicates, and to make sure that our database is 100% accurate with stripe objects(assuming the client doesn't mess with the metadata). check every webhook and function
